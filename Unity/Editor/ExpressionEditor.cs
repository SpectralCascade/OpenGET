...
namespace OpenGET.Editor
{
    ...
    public class ExpressionEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool modified = false;
            ExpressionField(modifier.modifier.expression, factory, ref modified, true, "Expression: ");
            modified = false;
            Expression changed = ExpressionField(modifier.modifier.expression, factory, ref modified);
            ...
            if (changed != null)
            {
                modifier.modifier.expression = changed;
            }
            else
            {
                modifier.modifier.expression = new Constant(new VariantInteger(0));
            }
        }
                ...
                switch (index)
                {
                    ...
                    case 1:
                        field = factory.Create(
                            Mathf.FloorToInt(oldIndex == 2 ? (float)field.value : (oldIndex == 3 && float.TryParse(field.value as string, out float oldFloat) ? oldFloat : 0f))
                        );
                        break;
                    // float
                    case 2:
                        field = factory.Create(
                            oldIndex == 1 ? (int)field.value : (oldIndex == 3 && float.TryParse(field.value as string, out float oldValue) ? oldValue : 0f)
                        );
                        break;
                    // string
                    case 3:
                        field = factory.Create(field.value.ToString());
                        break;
        ...
        public UnityVariable UnityVariableField(UnityVariable field, out Expression inner, ref bool changed)
        {
            ...
            field._target.reference = EditorGUILayout.ObjectField(field._target.reference, typeof(Object), allowSceneObjects: true);
            int index = 0;

            List<string> options = new List<string> { };
            System.Type targetType = field._target.reference != null ? field._target.reference.GetType() : null;
            ...
            if (targetType != null) {
                ...
                for (int i = 0, counti = fields.Length; i < counti; i++) {
                    options.Add(field._target.reference.name + "/" + fields[i].Name);
            ...
            {
                changed = true;
                if (targetType == null || index == options.Count - 1)
                {
                    // Delete
                    field = null;
                }
