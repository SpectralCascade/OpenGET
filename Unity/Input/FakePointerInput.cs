...
namespace UnityEngine.InputSystem.UI
{
    ...
    /// <seealso cref="Mouse"/>
    [AddComponentMenu("Input/Virtual Mouse [Fixed]")]
    public class FakePointerInput : MonoBehaviour
    {
        /// <summary>
        ...
        protected void OnEnable()
        {
            ...
            // IMPORTANT: Re-enable and re-pair with the default input user if available
            InputSystem.EnableDevice(m_VirtualMouse);
            if (PlayerInput.all.Count > 0)
            {
                UnityEngine.InputSystem.Users.InputUser.PerformPairingWithDevice(m_VirtualMouse, PlayerInput.all[0].user);
            }
        }
        ...
        protected void OnDisable()
        {
            ...
            {
                InputSystem.onAfterUpdate -= m_AfterInputUpdateDelegate;
                m_AfterInputUpdateDelegate = null;
            }
            ...
            if (m_ButtonActionTriggeredDelegate != null)
            {
                ...
                SetActionCallback(m_ForwardButtonAction, m_ButtonActionTriggeredDelegate, false);
                SetActionCallback(m_BackButtonAction, m_ButtonActionTriggeredDelegate, false);
                m_ButtonActionTriggeredDelegate = null;
            }
        ...
        private void UpdateMotion()
        {
            ...
            else
            {
                ...
                if (m_CursorTransform != null &&
                    ...
                    m_CursorTransform.anchoredPosition = ClampCanvas(newPosition);
        ...
        private RectTransform canvasRect {
            get {
                ...
                return m_Canvas?.transform as RectTransform;
            }
        }
        ...
        private Vector2 ClampCanvas(Vector2 position)
        {
            if (canvasRect == null)
            {
                return position;
            ...
            RectTransformUtility.ScreenPointToLocalPointInRectangle(canvasRect, position,
        ...
        private static void SetActionCallback(InputActionProperty field, Action<InputAction.CallbackContext> callback, bool install = true)
        {
            ...
            // This used to only used "started" and "cancelled" callbacks,
            // but "started" never actually seems to fire; whereas "performed" does.
