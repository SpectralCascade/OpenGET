Shader "OpenGET/Unlit/LightRayMesh"
{
    Properties
    {
        ...
        _Spread("Spreading Factor", float) = 1.0
        _Distance("Distance", float) = 1.0
        _Direction("Ray Direction Vector", Vector) = (0.0, 0.0, 1.0)
    ...
    Category
    {
        SubShader
        {
            ...
            Pass
            {
                ...
                uniform Vector _Direction;

                VertexOutput vertParticleUnlitModified(appdata_particles v)
                {
                    // Original vertex function
                    VertexOutput o;
                    vertParticleUnlit(v, o);

                    // Modify by distance, spread and direction
                    if (v.vertex.z != 0)
                    {
                        // Point in direction
                        Vector map = _Direction * _Distance;
                        v.vertex.x += map.x;
                        v.vertex.y += map.y;
                        v.vertex.z = map.z;

                        // Now spread and distance factors
                        v.vertex.x *= _Spread;
                        v.vertex.y *= _Spread;
                        //v.vertex.z *= _Distance;
                    }
                ...
                #pragma vertex vertParticleUnlitModified
    ...
    Fallback "VertexLit"
    CustomEditor "LightRayShaderEditor"
}
